...
def draw_overlay(frame, features, agent_state, frame_num, total_frames, fps, num_people):
    ...
    texts = [
        ...
        (f"Body Speed: {features.body_speed:.3f} u/s", (0, 200, 255) if features.body_speed > 0.15 else COLOR_WHITE),
        (f"Angle Rate: {features.angle_change_rate:.1f} d/s", (0, 200, 255) if features.angle_change_rate > 20.0 else COLOR_WHITE),
    ]

    # Dark background for text
    overlay2 = frame.copy()
    cv2.rectangle(overlay2, (0, banner_h), (260, banner_h + len(texts) * line_h + 15), COLOR_DARK, -1)
...
def process_video(
    ...
    while True:
        ...
        # Debug: print key features for tuning
        if frame_count % 10 == 0 or agent_state.state != AgentStateName.NORMAL:
            print(f"    f{frame_count:4d} | state={agent_state.state.value:20s} | conf={agent_state.confidence:.3f} "
                  f"| posture={features.posture_score:.3f} | speed={features.body_speed:.4f} "
                  f"| angRate={features.angle_change_rate:.1f} | motion={features.sudden_motion_change:.4f} "
                  f"| dCog={features.cumulative_delta_cog:.4f} | dAng={features.cumulative_delta_angle:.1f}")
